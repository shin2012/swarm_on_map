...
## 7. 향후 개선 아이디어 (TODO)
- **필터링 기능**: 카테고리별 혹은 날짜 범위별 데이터 필터링 추가.
- **검색**: 장소명(VENUE)으로 검색하여 해당 위치로 이동하는 기능.
- **성능 최적화**: 데이터가 더 늘어날 경우 서버 측에서 GeoJSON 형식으로 최적화하거나 클러스터링 옵션 조정.
- **UI 개선**: 다크 모드 지원 또는 더 세련된 팝업 디자인 적용.
